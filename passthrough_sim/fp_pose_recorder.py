...
def parse_args():
    ...
    p.add_argument("--init_min_depth", type=float, default=0.02,
                   help="Init-only: reject FP poses whose object depth (+Z in "
                        "the camera optical frame) is below this (m). Guards "
                        "against behind-camera / degenerate seeds that render "
                        "to a non-finite optimiser loss downstream.")
    p.add_argument("--init_max_depth", type=float, default=10.0,
                   help="Init-only: reject FP poses whose object depth exceeds "
                        "this (m).")
    return p.parse_args()
...
def _init_pose_sane(T, min_depth, max_depth):
    """Validate an FP init pose before we seed the optimiser with it.

    T is object-in-camera (optical frame, +Z forward). A bad-depth early
    frame can make FP emit a non-finite, non-SO(3), or behind-camera pose;
    seeding the IPCAI optimiser with that renders to a non-finite loss, the
    optimiser never commits a 'best' (NaN < inf is False), and the resulting
    uninitialised pose is singular when inverted. Reject those here so the
    handoff waits for a clean estimate. Returns (ok, reason).
    """
    ...
    if ortho_err > 1e-2 or not (0.9 < det < 1.1):
        return False, f"non-SO3 (ortho_err={ortho_err:.3g}, det={det:.3g})"
    z = float(T[2, 3])
    if not (min_depth <= z <= max_depth):
        return False, (f"implausible depth z={z:+.3f}m "
                       f"(want [{min_depth:g}, {max_depth:g}])")
    return True, "ok"
...
class PoseRecorder(Node):
    ...
    def __init__(self, args):
        ...
        self._init_min_depth = float(args.init_min_depth)
        self._init_max_depth = float(args.init_max_depth)
        self._init_skips = 0
    ...
    def _on_matrix(self, msg: TensorList):
        ...
        if self.init_only:
            ...
            sane, reason = _init_pose_sane(
                T_cam_centered, self._init_min_depth, self._init_max_depth)
            if not sane:
                # FP (estimation-only) emits a pose every frame; skip the bad
                # ones (typically early frames with marginal depth) and wait
                # for a clean estimate rather than seeding a degenerate pose.
                self._init_skips += 1
                if self._init_skips <= 3 or self._init_skips % 30 == 0:
                    self.get_logger().warn(
                        f"INIT: skipping unusable FP pose ({reason}); waiting "
                        f"for a clean estimate (skipped={self._init_skips})")
                return
            if self._init_skips:
                self.get_logger().info(
                    f"INIT: accepted clean FP pose after "
                    f"{self._init_skips} skip(s).")
