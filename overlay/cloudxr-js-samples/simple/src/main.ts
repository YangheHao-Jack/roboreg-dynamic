...
// --- robot overlay (renders the URDF on top of the CloudXR stream) ---
...
class CloudXRClient {
  ...
  private onSessionCallback?: (session: Session) => void;
  ...
  constructor(options?: {
    ...
    this.onSessionCallback = options?.onSession;
  ...
  private async createCloudXRSession(
    ...
    try {
      ...
      if (this.onSessionCallback) this.onSessionCallback(this.cloudxrSession);
    } catch (error) {
...
// 16-byte channel UUID — MUST match kChannelUuid in opaque_data_sender.cpp.
const CHANNEL_UUID = '526f626f5265670065787472696e7300';

function uuidHex(u: any): string {
  // Channel uuid may be a string, Uint8Array, or {bytes}. Normalize to lowercase hex.
  if (typeof u === 'string') return u.replace(/-/g, '').toLowerCase();
  const bytes = u?.bytes || u;
...
// Receive camera_to_base (and optional joints) over the CloudXR opaque data
// channel — same WebRTC connection as the video, so the extrinsic arrives
// aligned with the frames (no rosbridge hop, latency removed at the source).
// Wire protocol matches opaque_data_sender.cpp:
//   [0]=0x00 + 16×f32 row-major camera_to_base
//   [0]=0x01 + u8 count + count×f32 joint rads
function startPoseFeedDataChannel(session: Session, overlay: OverlayModule) {
  const pick = () => {
    const chans = (session as any).availableMessageChannels || [];
    return chans.find((c: any) => uuidHex(c.uuid) === CHANNEL_UUID) || chans[0] || null;
  };

  (async () => {
    let channel = null;
    // Channels appear shortly after connect — poll briefly.
    for (let i = 0; i < 100 && !channel; i++) {
      channel = pick();
      if (!channel) await new Promise((r) => setTimeout(r, 100));
    }
    if (!channel) { console.error('[overlay] no opaque data channel found'); return; }
    console.info('[overlay] data channel open:', uuidHex(channel.uuid));

    while (true) {
      const data = await channel.receiveMessage();   // Uint8Array | null
      if (data === null) { console.warn('[overlay] data channel closed'); break; }
      if (!data.length) continue;
      const dv = new DataView(data.buffer, data.byteOffset, data.byteLength);
      const type = dv.getUint8(0);
      if (type === 0x00 && data.byteLength >= 1 + 16 * 4) {
        const m = new Array(16);
        for (let i = 0; i < 16; i++) m[i] = dv.getFloat32(1 + i * 4, true); // little-endian
        overlay.setPoseFromArray(m);                  // 16 row-major = camera_to_base
      } else if (type === 0x01 && data.byteLength >= 2) {
        const n = dv.getUint8(1);
        const j = new Array(n);
        for (let i = 0; i < n; i++) j[i] = dv.getFloat32(2 + i * 4, true);
        overlay.setJoints(j);
      }
    }
  })();
...
// ── transport selector ─────────────────────────────────────────────────────
// 'datachannel' — CloudXR opaque data channel (sim validation): frame-aligned,
//                 no extra latency, head-speed gate OFF (live tracking).
// 'rosbridge'   — pose from the consumer (roboreg) over wss, for real
//                 passthrough: latency present, gate ON. In production the host
//                 is the standalone passthrough viewer, but this feed function
//                 and the overlay config below port to it unchanged.
type Transport = 'datachannel' | 'rosbridge';
const ROSBRIDGE_URL = 'wss://192.168.1.154:9090';

// Runtime transport selection — read at session start, so set it BEFORE you hit
// Connect. Priority: URL "?transport=" > the on-page dropdown > default.
// Returning Transport (the union) also keeps both === comparisons type-valid.
function getTransport(): Transport {
  const p = new URLSearchParams(location.search).get('transport');
  if (p === 'rosbridge' || p === 'datachannel') return p;
  const el = document.getElementById('overlay-transport') as HTMLSelectElement | null;
  if (el && (el.value === 'rosbridge' || el.value === 'datachannel')) return el.value;
  return 'datachannel';   // default
}

// Which pose drives the mesh overlay (rosbridge transport only). Two separate
// topics coexist so GT and the estimate never collide on one topic:
//   'gt'        — the bridge's render-derived camera-in-base on /xr/extrinsic_left
//                 (xr_ros_bridge publish_capture; the per-frame "ground truth")
//   'estimated' — the consumer's optimiser result on /xr/extrinsic_left_estimated
//                 (stereo_pipeline_export / passthrough_consumer --publish-estimated-pose)
...
function getPoseTopic(): string {
  ...
  if (p === 'gt')        return POSE_TOPIC_GT;
  const el = document.getElementById('overlay-pose') as HTMLSelectElement | null;
  if (el && el.value === 'estimated') return POSE_TOPIC_ESTIMATED;
  if (el && el.value === 'gt')        return POSE_TOPIC_GT;
  return POSE_TOPIC_GT;   // default: GT (preserves current behavior)
}
...
async function startPoseFeedRosbridge(overlay: OverlayModule) {
  const ROSLIB: any = await import('roslib');
  const ros = new ROSLIB.Ros({ url: ROSBRIDGE_URL });
  ros.on('connection', () => console.info('[overlay] rosbridge connected'));
  ros.on('error', (e: any) => console.error('[overlay] rosbridge error', e));
  ...
  }).subscribe((m: any) => overlay.setPoseFromPosQuat(m.pose.position, m.pose.orientation));
  new ROSLIB.Topic({
    ros, name: '/xr/joint_states', messageType: 'sensor_msgs/JointState',
    throttle_rate: 0, queue_length: 1,
  }).subscribe((m: any) => overlay.setJoints(m.position, m.name));
}

document.addEventListener('DOMContentLoaded', () => {
  // World-locked bake-and-hold (the module's only mode). Always-bakes on each
  // registration update — relies on a low-latency feed (data channel here;
  // a direct roboreg→Quest channel in passthrough) rather than a head gate.
  const overlay = new OverlayModule();
  (window as any).overlay = overlay;   // console access

  // On-page transport picker (top-left). Choose BEFORE pressing Connect;
  // it's read when the XR session starts. URL "?transport=…" overrides it.
  if (!document.getElementById('overlay-transport')) {
    const wrap = document.createElement('div');
    wrap.style.cssText =
      'position:fixed;top:8px;left:8px;z-index:99999;background:rgba(0,0,0,.6);' +
      'color:#fff;padding:6px 8px;border-radius:6px;font:13px sans-serif';
    wrap.textContent = 'Overlay feed: ';
    const sel = document.createElement('select');
    sel.id = 'overlay-transport';
    for (const v of ['datachannel', 'rosbridge']) {
      const o = document.createElement('option');
      o.value = v; o.textContent = v; sel.appendChild(o);
    }
    const p = new URLSearchParams(location.search).get('transport');
    if (p === 'rosbridge' || p === 'datachannel') sel.value = p;  // reflect URL override
    wrap.appendChild(sel);

    // Pose-source picker (rosbridge only): which topic drives the mesh.
    // gt = bridge render pose (/xr/extrinsic_left); estimated = optimiser
    // result (/xr/extrinsic_left_estimated). URL "?pose=…" overrides it.
    ...
    poseSel.id = 'overlay-pose';
    for (const v of ['gt', 'estimated']) {
      const o = document.createElement('option');
      o.value = v; o.textContent = v; poseSel.appendChild(o);
    }
    const pp = new URLSearchParams(location.search).get('pose');
    if (pp === 'gt' || pp === 'estimated') poseSel.value = pp;  // reflect URL override
    wrap.appendChild(poseSel);

    document.body.appendChild(wrap);
  }

  new CloudXRClient({
    onWebGLInitialized: async (gl, _useQuestColorWorkaround, referenceSpace) => {
    ...
    onSession: (session) => {
      if (getTransport() === 'datachannel') startPoseFeedDataChannel(session, overlay);
    },
